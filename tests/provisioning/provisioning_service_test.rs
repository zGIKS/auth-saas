...
async fn test_provision_tenant_resources_success() {
    ...
    mock_provisioner.expect_create_schema()
        .withf(|name| name == "tenant_acme")
        .times(1)
        .returning(|_| Ok(()));

    mock_provisioner.expect_run_migrations()
        .withf(|name| name == "tenant_acme")
        .times(1)
        .returning(|_| Ok(()));

    let service = ProvisioningCommandServiceImpl::new(mock_provisioner);
    let command = ProvisionTenantResourcesCommand::new("id-123".to_string(), "tenant_acme".to_string()).unwrap();
...
async fn test_deprovision_tenant_resources_success() {
    ...
    mock_provisioner.expect_drop_schema()
        .withf(|name| name == "tenant_acme")
        .times(1)
        .returning(|_| Ok(()));

    let service = ProvisioningCommandServiceImpl::new(mock_provisioner);
    let command = DeprovisionTenantResourcesCommand::new("id-123".to_string(), "tenant_acme".to_string()).unwrap();

    ...
    P: ProvisioningFacade,
{
    pub fn new(repository: R, provisioning_facade: P, jwt_secret: String) -> Self {
        Self { repository, provisioning_facade, jwt_secret }
    }
}
    ...
    P: ProvisioningFacade,
{
    async fn create_tenant(&self, command: CreateTenantCommand) -> Result<(Tenant, String), TenantError> {
        ...
        let schema_name = match &tenant.db_strategy {
             crate::tenancy::domain::model::value_objects::db_strategy::DbStrategy::Shared { schema } => schema.clone(),
        };

        self.provisioning_facade.provision_tenant(
            tenant.id.value().to_string(),
            schema_name,
        ).await.map_err(|e| TenantError::InfrastructureError(e.to_string()))?;
    ...
    async fn delete_tenant(&self, command: DeleteTenantCommand) -> Result<(), TenantError> {
        ...
        // 1. Find Tenant
        let tenant = self.repository.find_by_id(&tenant_id_vo).await?
        ...
        self.provisioning_facade.deprovision_tenant(
            tenant.id.value().to_string(),
            schema_name,
        ).await.map_err(|e| TenantError::InfrastructureError(e.to_string()))?;
